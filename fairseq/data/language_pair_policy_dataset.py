...
def collate(
    ...
    is_true_target_list = isinstance(samples[0]['true_target'], list)
    return {
        ...
        'true_target': merge('true_target', is_true_target_list),
        'nsentences': samples[0]['enc_source'].size(0),
    }


class LanguagePairSelfDatasetPolicy(FairseqDataset):
    """
    ...
    def __getitem__(self, index):
        enc_source, dec_source, dec_target, true_target, ntokens = self._make_source_target(self.src[index], self.tgt[index])
        return {'id': index, 'enc_source': enc_source, 'dec_source': dec_source, 'dec_target': dec_target, 'true_target': true_target, 'ntokens': ntokens}

    def __len__(self):
        return len(self.src)

    def set_random_refine_step(self, max_step, force_refine_step=None):
        self.max_refine_step = max_step
        if force_refine_step is not None:
            self.random_refine_step = force_refine_step
        else:
            if getattr(self.args, "focus", -1) >= 0:
                self.random_refine_step = self.args.focus
            else:
    ...
    def _make_source_target(self, source, target):
        ...
        if self.train:
            if min_num_masks < len(dec_source):
                if self.progressive:
                    sample_size = int((1.0 - (self.random_refine_step / self.max_refine_step)) * len(dec_source))
                    sample_size = max(min_num_masks, sample_size)
                else:
        ...
        return enc_source, dec_source, dec_target, dec_target_cp, ntokens
    ...
    def collater(self, samples):
        ...
        if self.progressive and "net_input" in ret_samples:
            ret_samples["random_refine_step"] = self.random_refine_step
            ret_samples["max_refine_step"] = self.max_refine_step
        return ret_samples
    ...
    def get_dummy_batch(self, num_tokens, max_positions, src_len=128, tgt_len=128):
        ...
        enc_source, dec_source, dec_target, true_target, ntokens = self._make_source_target(self.src_dict.dummy_sentence(src_len), self.tgt_dict.dummy_sentence(tgt_len))
